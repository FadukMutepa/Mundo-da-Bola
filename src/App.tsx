...
export default function App() {
  ...
  // --- INITIALIZE NEWS FROM LOCAL STORAGE OR RAW DATA ---
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        console.error('Error loading news from storage, reverting to default.', e);
        setNews(INITIAL_NEWS);
        localStorage.setItem('futebol_news', JSON.stringify(INITIAL_NEWS));
      }
    } else {
      setNews(INITIAL_NEWS);
      localStorage.setItem('futebol_news', JSON.stringify(INITIAL_NEWS));
  ...
  const feedNews = useMemo(() => {
    if (selectedCategory === 'Todos' && !searchQuery) {
      // Exclude main highlight only under clean default view
  ...
  const handleLike = (articleId: string, e?: React.MouseEvent) => {
    ...
    const updated = news.map(item => {
      if (item.id === articleId) {
        const likedItem = { ...item, likes: item.likes + 1 };
        // If it's the currently viewed article, sync detail view
        if (selectedArticle && selectedArticle.id === articleId) {
          setSelectedArticle(likedItem);
        }
        return likedItem;
      }
      return item;
  ...
  const handlePostComment = (e: React.FormEvent) => {
    ...
    const updated = news.map(item => {
      if (item.id === selectedArticle.id) {
        ...
        setSelectedArticle(withComment);
        return withComment;
      }
      return item;
    });
    ...
    setCommentName('');
    setCommentBody('');
    setSuccessToast('Comentário enviado com sucesso!');
  };
  ...
  const handleCreateNews = (e: React.FormEvent) => {
    ...
    const finalImage = formCustomImage.trim() || formImagePreset;
    ...
    // If marked as Highlight, reset other Highlights
    if (formIsHighlight) {
      updatedList = updatedList.map(item => ({ ...item, isHighlight: false }));
    }

    // Prepend new article to the database
    ...
    setSuccessToast('Nova notícia publicada com sucesso!');
    
    // Jump view to respect the uploaded category
          ...
          <div className="fixed inset-0 z-50 flex justify-end" id="publisher_drawer_container">
            ...
            >
              ...
              <form onSubmit={handleCreateNews} className="flex-grow p-6 overflow-y-auto space-y-5" id="form_create_article">
                ...
                {/* 6. Image Selection (Presets) */}
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="block text-xs font-semibold text-slate-600 uppercase tracking-widest">Selecione uma Imagem do Banco</label>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    {IMAGE_PRESETS.map((preset) => (
                      <div 
                        key={preset.name}
                        onClick={() => { setFormImagePreset(preset.url); setFormCustomImage(''); }}
                        className={`group relative h-16 rounded-lg overflow-hidden cursor-pointer border-2 transition-all ${
                          formImagePreset === preset.url && !formCustomImage
                  ...
                  {/* Or Custom Image link input */}
                  <div>
                    <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Ou cole o link de uma imagem externa da sua escolha</label>
                    <input
                      type="url"
                      placeholder="https://exemplo.com/sua-imagem.jpg"
                      value={formCustomImage}
                      onChange={(e) => setFormCustomImage(e.target.value)}
                      className="w-full bg-slate-50 border border-slate-200 focus:bg-white focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-lg px-3 py-1.5 text-xs outline-none transition-all"
                    />
                  </div>
                </div>
